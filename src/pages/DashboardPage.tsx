  ...
  return (
    <div className="p-8">
      ...
      <div className="flex gap-2 mb-6">
        {([
          { key: 'overview' as ViewMode, label: 'All-market heatmap', icon: '🔥', sub: 'Scan all markets × all pillars' },
          { key: 'portfolio' as ViewMode, label: 'Portfolio fit', icon: '🎯', sub: 'Custom-weighted match' },
          { key: 'compare' as ViewMode, label: 'Compare markets', icon: '⚖️', sub: 'Up to 5 side-by-side' },
        ]).map(t => (
          <button
            key={t.key}
            onClick={() => setView(t.key)}
            className={`px-4 py-3 rounded-xl text-left transition-all ${
              view === t.key ? 'text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-200 hover:border-purple-300'
            }`}
            style={view === t.key ? { background: '#3B1F6B' } : {}}
          >
            <div className="flex items-center gap-2 font-semibold text-sm">
              <span>{t.icon}</span>
              {t.label}
            </div>
            <div className={`text-xs mt-0.5 ${view === t.key ? 'opacity-80' : 'text-gray-400'}`}>
              {t.sub}
            </div>
          </button>
        ))}
      ...
      {/* Views */}
      {view === 'overview' && <AllMarketsHeatmap markets={allMarkets} />}
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${
                      isSelected ? 'text-white border-transparent' : 'text-gray-600 border-gray-200 hover:border-purple-300 bg-white'
                    }`}
          ...
          {selectedMarkets.length === 0 ? (
            ...
            <>
                ...
                {selectedMarkets.map((sm, i) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="text-xs font-bold uppercase tracking-wide mb-0.5" style={{ color: COLOURS[i % COLOURS.length] }}>
                        ...
                        <div key={p.name} className="flex items-center gap-2">
                          ...
                          <div className="flex-1 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                            <div
                              ...
                              style={{ width: `${((sm.pillarScores[p.name]?.score ?? 0) / 5) * 100}%`, background: COLOURS[i % COLOURS.length] }}
                            />
                          </div>
              ...
              <div className="bg-white rounded-xl border border-purple-100 shadow-sm p-5">
                  ...
                  {(['radar', 'bars', 'heatmap'] as const).map(t => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors capitalize ${
                        chartTab === t ? 'text-white' : 'text-gray-500 hover:bg-purple-50'
                      }`}
                      style={chartTab === t ? { background: '#3B1F6B' } : {}}
                    >
                ...
                {chartTab === 'bars'  && <PillarBars markets={selectedMarkets} />}
